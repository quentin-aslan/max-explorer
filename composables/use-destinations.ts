...
export const useDestinations = () => {
  ...
  const fetchDestinations = async (
    ...
    try {
      ...
      destinations.value = []
      const formattedDepartureDate = departureDate ? toISOStringWithOffset(departureDate).slice(0, 10) : ''
      const formattedReturnDate = returnDate ? toISOStringWithOffset(returnDate).slice(0, 10) : ''
